...
if ( ! empty( $full_height ) ) {
	$height_class = ' vc_row-o-full-height';
	if ( ! empty( $content_placement ) ) {
		$heightcontent_class = ' vc_row-o-content-' . $content_placement;
	}
}
...
if($video_bg) {
	...
	if($enable_video_color_overlay == 'true'){
		$out_escaped .=  '<div class="video-overlay" style="background-color: '. esc_attr($video_overlay_color) .';"></div>';
	}
	
	if($video_image){
		$video_image_src = wp_get_attachment_image_src($video_image, 'full');
		$out_escaped .=  '<div class="video-fallback" style="background-image: url('. esc_url($video_image_src[0]) .')"></div>';
	}
	
	$out_escaped .= '
	<div class="video-wrap">
		<video width="1920" height="800" preload="auto" autoplay loop muted="muted">';
		    if(!empty($video_webm)) { $out_escaped .= '<source src="'. esc_url($video_webm) .'" type="video/webm">'; }
		    if(!empty($video_mp4)) { $out_escaped .= '<source src="'. esc_url($video_mp4) .'" type="video/mp4">'; }
		    if(!empty($video_ogv)) { $out_escaped .= '<source src="'. esc_url($video_ogv) .'" type="video/ogg">'; }
	$out_escaped .='
		</video>
	</div>';
	
	echo $out_escaped; // WPCS: xss ok.
}
