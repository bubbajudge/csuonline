...
class ITSEC_Admin_Notice_New_Feature_Core implements ITSEC_Admin_Notice {
	...
	public function get_message() {
		if ( ITSEC_Core::is_pro() ) {
			return sprintf(
				esc_html__( 'The new, improved %1$sWordPress Security Site Scan%2$s powered by iThemes performs automatic checks for known website vulnerabilities and, if a patch is available, iThemes Security Pro will now automatically apply the fix for you.', 'it-l10n-ithemes-security-pro' ),
				'<a href="{{ $blog }}">',
				'</a>'
			);
		}
		...
		return sprintf(
			esc_html__( 'The new, improved %1$sWordPress Security Site Scan powered by iThemes checks if Google has detected malware and added your site to their threat list', 'it-l10n-ithemes-security-pro' ),
			'<a href="{{ $blog }}">',
			'</a>'
		);
	}
