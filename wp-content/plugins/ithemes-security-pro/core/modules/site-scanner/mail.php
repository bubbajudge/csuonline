...
class ITSEC_Site_Scanner_Mail {
	...
	public static function send( $scan ) {
		if ( ! $scan instanceof Scan ) {
			_doing_it_wrong( __METHOD__, __( 'Must pass Scan instance.', 'it-l10n-ithemes-security-pro' ), '5.8.1' );
	...
	public static function format_scan_body( ITSEC_Mail $mail, $scan ) {
		...
		if ( $scan->is_error() ) {
			$mail->add_list( array(
				...
				sprintf( esc_html__( 'Error Message: %s', 'it-l10n-ithemes-security-pro' ), $scan->get_error()->get_error_message() ),
				sprintf( esc_html__( 'Error Code: %s', 'it-l10n-ithemes-security-pro' ), '<code>' . esc_html( $scan->get_error()->get_error_code() ) . '</code>' ),
			), true );

			if ( $log_url ) {
				$mail->add_button( esc_html__( 'View Report', 'it-l10n-ithemes-security-pro' ), $log_url );
			}
